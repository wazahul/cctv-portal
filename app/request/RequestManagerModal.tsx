...
export default function RequestManagerModal({ isOpen, onClose, onRefresh, filterSn }: any) {
  ...
  // 🚩 FIXED: Cross-Browser Compatible WhatsApp Dispatch
  const handleAction = async (req: any) => {
  // 🚩 STEP 1: WhatsApp URL pehle hi prepare kar lein
  ...
  try {
    // 🚩 STEP 2: Safari bypass - Database update ko "Background" mein bhejein
    // Hum await ka intezar nahi karenge, seedha redirect karenge
    supabase.from("requests").update({ status: "done" }).eq("id", req.id).then(() => {
        fetchRequests(); 
        onRefresh();
    });

    // 🚩 STEP 3: Sabse important - Redirect immediate hona chahiye
    // Safari ise 'User-Initiated' tabhi manega jab ye stack ke upar ho
    window.location.href = waUrl;

    if (filterSn) onClose();
  } catch (err) {
    console.error("Safari Redirect Error:", err);
    // Fallback agar redirection fail ho
    ...
    link.click();
      ...
      // 🛠️ BROWSERS FIX: Create a temporary anchor to bypass popup blockers
      const link = document.createElement("a");
      link.href = waUrl;
      ...
      console.error(err); 
      setDialog({ isOpen: true, title: "Error", message: "Failed to dispatch WhatsApp message.", type: "danger" });
    }
  };
