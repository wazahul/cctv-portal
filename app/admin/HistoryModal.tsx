        ...
        <div className="w-full max-w-xl bg-white sm:rounded-[40px] shadow-2xl overflow-hidden relative animate-in slide-in-from-bottom duration-500 h-full sm:h-auto sm:max-h-[90vh] flex flex-col overscroll-none">
          ...
          <div className="p-6 border-b border-slate-100 flex justify-between items-center bg-white sticky top-0 z-10 shrink-0 text-left">
            ...
            <div className="flex gap-2">
              {logs.length > 0 && (
                <button onClick={downloadMasterPDF} className="p-3 bg-slate-900 text-white rounded-2xl active:scale-90 shadow-lg"><FileText size={18} /></button>
              )}
              <button onClick={onClose} className="p-3 bg-slate-50 rounded-2xl text-slate-400 active:scale-90 border border-slate-100"><X size={20} strokeWidth={3} /></button>
            </div>
          </div>

          {/* BODY (No UI Changes) */}
          <div className="p-5 sm:p-8 overflow-y-auto custom-scroll space-y-5 bg-slate-50/50 flex-1 pb-24 touch-pan-y" style={{ overscrollBehavior: 'contain' }}>
            {loading ? (
              ...
              logs.map((log) => (
                <div key={log.id} className="relative p-5 rounded-[30px] border border-slate-200 bg-white shadow-sm active:scale-[0.98] transition-all group">
                  <div className="flex justify-between items-start mb-4">
                    ...
                    <button onClick={(e) => { e.stopPropagation(); downloadSingleReceipt(log); }} className="p-2.5 bg-emerald-50 text-emerald-600 rounded-xl active:scale-90 border border-emerald-100 shadow-sm"><Printer size={16} /></button>
                  </div>
                  ...
                  <div className="space-y-4 text-left">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <p className="text-slate-400 font-black text-[8px] uppercase tracking-widest leading-none">Technician</p>
                        <p className="text-sm font-bold text-slate-800 uppercase mt-1">{log.technician_name}</p>
                      </div>
                    </div>
                    ...
                    <div className="flex items-start gap-3">
                      ...
                      <div>
                        <p className="text-slate-400 font-black text-[8px] uppercase tracking-widest leading-none">Work Done</p>
                        <p className="text-sm text-slate-700 font-medium leading-tight mt-1">{log.work_done}</p>
                      </div>
                    </div>

                    <div className="flex items-start gap-3">
                      <div className="bg-purple-50 p-2.5 rounded-xl text-purple-600 mt-0.5 text-left"><MessageSquare size={16} /></div>
                      <div>
                        <p className="text-[8px] font-black text-slate-400 uppercase tracking-[2px] leading-none">Engineer Remarks</p>
                        <p className="text-sm text-slate-600 italic font-bold mt-1.5 leading-relaxed">"{log.remarks || 'No special remarks provided.'}"</p>
                      </div>
                    </div>
              ...
              <div className="py-20 text-center opacity-30 flex flex-col items-center gap-4">
                 <AlertCircle size={48} />
                 <p className="font-black text-[10px] uppercase tracking-[4px]">No History Records</p>
              </div>
            )}
          </div>
        </div>
